    ...
    for i in range(ngroup):
        if scale1 is None:
            ...
            if dimy == 96:
                ...
                # initial values from ALbert et al. 2023
                xprel, xpreu = 200, 240
                yprel, ypreu = 300, 500
                xpostl, xpostu = 135, 215
                ypostl, ypostu = 1000, 1100
            pre_bkg_ratio = deepstack[i, xprel:xpreu, yprel:ypreu] / background_model[xprel:xpreu, yprel:ypreu]
            post_bkg_ratio = deepstack[i, xpostl:xpostu, ypostl:ypostu] / background_model[xpostl:xpostu, ypostl:ypostu]
            # Instead of a straight median, use the median of the 2nd quartile
            # to limit the effect of any remaining illuminated pixels.
            preq1 = np.nanpercentile(pre_bkg_ratio, 25)
            preq2 = np.nanpercentile(pre_bkg_ratio, 50)
            preii = np.where((pre_bkg_ratio > preq1) & (pre_bkg_ratio < preq2))
            scale1 = np.nanmedian(pre_bkg_ratio[preii])
            if scale1 < 0:
                scale1 = 0
            print('  Pre-step background scale factor: {1:.5f}'.format(i + 1, scale1))
            postq1 = np.nanpercentile(post_bkg_ratio, 25)
            postq2 = np.nanpercentile(post_bkg_ratio, 50)
            postii = np.where((post_bkg_ratio > postq1) & (post_bkg_ratio < postq2))
            scale1 = np.nanmedian(post_bkg_ratio[postii])
            if scale2 < 0:
                scale2 = 0
            print('  Post-step background scale factor: {1:.5f}'.format(i + 1, scale2))
